...
- **Market regime + sector rotation (no AI cost):** SPY/QQQ/IWM/VIX trend-and-volatility regime with a risk-off flag, market breadth, and per-sector rotation states (leading / improving / weakening / lagging vs SPY) from the 11 SPDR sector ETFs. Ideas are presented **by sector**, and every AI judgment receives this context.
...
- **Tabbed dashboard:** Portfolio · Market · Ideas · Analyze · Backtest · Chat · Activity, deep-linkable via URL hash, with unread badges on background tabs.
...
Optional: Telegram push (bot via @BotFather + chat id from @userinfobot in `.env`), Robinhood link (`bun run link:robinhood` — read-only, stores tokens locally; complete the SMS code or approve the login in the Robinhood app when prompted). Robinhood has no official public API; this uses the same private endpoints robin_stocks does (including the device-approval workflow) and can break without notice — the app degrades to import/YAML if so.
...
Yahoo benchmarks (SPY/QQQ/IWM/VIX + 11 sector ETFs) ─► regime + sector rotation + breadth
...
- `GET /api/market` — regime, benchmarks, sector rotation boards
